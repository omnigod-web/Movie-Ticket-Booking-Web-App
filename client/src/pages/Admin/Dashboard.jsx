...
const Dashboard = () => {
    ...
    // console.log(shows)
    console.log('dashboardData:', dashboardData.activeShows);
    
    return !loading ? (
        <>
            <Title text1='Admin' text2='Dashboard' />
            <div className="realtive flex flex-wrap gap-4 mt-6">
                    ...
                    <div key={show._id} className="w-55 rounded-lg overflow-hidden h-full pb-3 bg-primary/10 border border-primary/20 hover:translate-y-1 transition duration-300 ">
                        ...
                        <div className="flex items-center justify-between px-2">
                            ...
                            <p className="flex items-center gap-1 text-sm text-gray-400 mt-1 pr-1">
                                ...
                                {Number.isFinite(show.movie?.vote_average)
                                    ? show.movie.vote_average.toFixed(1)
                                    : 'N/A'}
                            </p>
                        </div>
                        <p className="px-2 pt2 text-sm text-gray-500"> {dateFormat(show.showDateTime)} </p>
                    </div>
                ))}
            </div>
        </>
